...
acceptance-frontend-image-build: ## Build Acceptance frontend server image
	...
	@docker build frontend -t fhabiana-machado/portal-governo-frontend:acceptance -f frontend/Dockerfile --build-arg VOLTO_VERSION=$(VOLTO_VERSION)

.PHONY: acceptance-backend-image-build
acceptance-backend-image-build: ## Build Acceptance backend server image
	@echo "Build acceptance backend"
	@docker build backend -t fhabiana-machado/portal-governo-backend:acceptance -f backend/Dockerfile.acceptance --build-arg PLONE_VERSION=$(PLONE_VERSION)
...
acceptance-frontend-container-start: ## Start Acceptance frontend container
	...
	@docker run --rm -p 3000:3000 --name portal-governo-frontend-acceptance --link portal-governo-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d fhabiana-machado/portal-governo-frontend:acceptance

.PHONY: acceptance-backend-container-start
acceptance-backend-container-start: ## Start Acceptance backend container
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name portal-governo-backend-acceptance -d fhabiana-machado/portal-governo-backend:acceptance
